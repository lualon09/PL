...
(func $preMain 
 ...
 ;; generating code of function calculaMax
(func $calculaMax
 (result i32)
 (local $temp i32)
 (local $localsStart i32)
 ...
 i32.add
 local.set $localsStart
 ;;generating code for declaration dec:max(type:INT)=AArray (AVar(arr)[(INT:0)])
 ...
 block
 loop
 ;;generating code for exp ebinLESS(AVar(i),AVar(length))
 ...
 ;;generating code for exp ebinGREATER(AArray (AVar(arr)[AVar(i)]),AVar(max))
 ;;generating code for access
 ...
 i32.gt_s
 if
 ;;generating code for assignation assign:AVar(max)=AArray (AVar(arr)[AVar(i)])
 ;;generating code for index of arrayAVar(i)
 ;;generating code for access
 i32.const 28
 local.get $localsStart
 ...
 i32.const 1
 call $copyn
 ;;end generating code for assignation assign:AVar(max)=AArray (AVar(arr)[AVar(i)])
 end
...
(func $main
 ...
 i32.store
 ;;end generating code for declaration
 ;;generating code for declaration dec:sol(type:INT)=call:calculaMax([AVar(array), AVar(length)])
 i32.const 24
 local.get $localsStart
 i32.add
 global.get $SP
